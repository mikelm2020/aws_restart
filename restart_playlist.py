...
def find_top_10_tracks(artist_ID):
...
# def create_playlist(user_id, list_track_id, playlist_name):
#     # Crea la playlist en Spotify
#     playlist_id = spotify_object.user_playlist_create(
...
#     spotify_object.playlist_add_items(playlist_id["id"], list_track_id)
...
while True:
    ...
    if choice == "0":
        ...
        print()
        artist_ID = find_artist(search_query)
        print("*" * 30, end="TOP 10" + ("*" * 44) + "\n")
        # Obtiene el top 10 del artista capturado
        artist_top_10_tracks = find_top_10_tracks(artist_ID)
        print()
        print("*" * 80)
        ...
        if len(list_track_ID) >= 2:
            ...
            #         create_playlist(user_ID, list_track_ID, playlist_name)
